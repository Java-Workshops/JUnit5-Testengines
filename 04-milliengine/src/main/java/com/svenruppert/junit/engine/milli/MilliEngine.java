...
public class MilliEngine
  ...
  private Predicate<Class<?>> checkClass() {
    return classCandidate ->
        match(
            ...
            matchCase(() -> isAnnotated(classCandidate, MilliTestClass.class), () -> success(Boolean.TRUE)))
  ...
  private void appendTestInMethod(Method javaMethod, EngineDescriptor rootNode) {
    ...
    if (checkClass().test(declaringClass))
      rootNode.addChild(
        new MilliEngineMethodTestDescriptor(
            javaMethod,
            declaringClass,
  ...
  private void appendTestInClass(Class<?> javaClass, EngineDescriptor rootNode) {
    if (checkClass().test(javaClass))
      rootNode.addChild(new MilliEngineClassTestDescriptor(javaClass, rootNode));
  }

  private void appendTestInPackage(String packageName, EngineDescriptor rootNode) {
    findAllClassesInPackage(packageName, checkClass(), name -> true)
        .stream()
        .peek((e) -> logger().info("class in package -> {}", e.getSimpleName()))
        .map(javaClass -> new MilliEngineClassTestDescriptor(javaClass, rootNode))
